...
func verifyLineSet(t *SmokeTestT,
	chip *gpioioctl.GPIOChip,
	lsTest *gpioioctl.LineSet,
	direction gpioioctl.LineDir,
	lines []string) {
	...
	for ix, lineName := range lines {
		...
		if lsl.Direction() != direction {
			t.Errorf("Expected LineSetLine.direction=%s, found %s", gpioioctl.DirectionLabels[direction], gpioioctl.DirectionLabels[lsl.Direction()])
		}
		s := lsl.String()
		if len(s) == 0 {
...
func createLineSets(t *SmokeTestT, chip *gpioioctl.GPIOChip, edge gpio.Edge) (lsOutput *gpioioctl.LineSet, lsInput *gpioioctl.LineSet) {
	// Create the Output Lineset
	lsOutput, err := chip.LineSet(gpioioctl.LineOutput, gpio.NoEdge, gpio.PullNoChange, outputLines...)
	if err != nil {
		t.Fatalf("Error creating output LineSet %s", err.Error())
	}

	verifyLineSet(t, chip, lsOutput, gpioioctl.LineOutput, outputLines)

	// Create the Input LineSet
	lsInput, err = chip.LineSet(gpioioctl.LineInput, edge, gpio.PullUp, inputLines...)
	if err != nil {
		t.Fatalf("Error creating input LineSet %s", err.Error())
	}

	verifyLineSet(t, chip, lsInput, gpioioctl.LineInput, inputLines)
	return
}

// Test Creating the line set and verify the pin setups.
func TestLineSetCreation(t *SmokeTestT) {
...
func TestLineSetReadWrite(t *SmokeTestT) {
	chip := gpioioctl.Chips[0]
	lsOutput, lsInput := createLineSets(t, chip, gpio.NoEdge)
...
func clearLineSetEdges(ls *gpioioctl.LineSet) bool {
	result := false
	for {
...
func TestLineSetWaitForEdgeTimeout(t *SmokeTestT) {
	lsOutput, lsInput := createLineSets(t, gpioioctl.Chips[0], gpio.RisingEdge)
	lsOutput.Close()
	defer lsInput.Close()
...
func TestLineSetHalt(t *SmokeTestT) {
	chip := gpioioctl.Chips[0]
...
// expected too.
func TestLineSetWaitForEdge(t *SmokeTestT) {
	...
	for _, test := range tests {
		lsInput, err := chip.LineSet(gpioioctl.LineInput, test.edgeSet, gpio.PullUp, inputLines...)
		if err != nil {
			t.Error(err)
			return
		}
		for ix, line := range lsOutput.Lines() {
			...
			if err == nil {
				...
				if edge != test.expectedEdge {
					t.Errorf("Test: %#v expected edge: %s received edge: %s", test, gpioioctl.EdgeLabels[test.expectedEdge], gpioioctl.EdgeLabels[edge])
				}
...
func TestLineSetConfigWithOverride(t *SmokeTestT) {
	chip := gpioioctl.Chips[0]
	...
	cfg := gpioioctl.LineSetConfig{
		...
		DefaultDirection: gpioioctl.LineOutput,
		DefaultEdge:      gpio.NoEdge,
		DefaultPull:      gpio.PullNoChange,
	}
	err := cfg.AddOverrides(gpioioctl.LineInput, gpio.RisingEdge, gpio.PullUp, []string{line1.Name()}...)
	if err != nil {
	...
	if lsl.Direction() != gpioioctl.LineOutput {
	...
	if lsl.Direction() != gpioioctl.LineInput {
